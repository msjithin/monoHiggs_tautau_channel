...
{
  TChain *chain = new TChain("tt/final/Ntuple");
  //eventHist = new TH1F("skimCounter", "skimCounter", 1, -0.5, 0.5);
  eventCount = 0;
  //TH1F *tmpHist;
  //TFile* tmpFile;
  ...
  while ((filename = (TSystemFile*)next()) && fileNumber >  maxFiles)
    {
      //if(fileNumber > 1)
	{
	  TString dataset = "make_ntuples_cfg";
	  ...
	  if(name.Contains(dataset))
	    {
	      ...
	      //TFile* tmpFile= TFile::Open(FullPathInputFile);
	      TFile *tmpFile = new TFile(FullPathInputFile);
	      TH1F *tmpHist = (TH1F*)tmpFile->Get("tt/skimCounter");
	      //cout<<FullPathInputFile<< "   tmpHist integral ="<<tmpHist->Integral()<<endl;
	      //eventHist->Add(tmpHist);
	      eventCount = eventCount + tmpHist->Integral();
	      tmpHist->Clear();
	      tmpFile->Close();
  ...
  //cout<<"eventHist ="<<eventHist->Integral()<<endl;
