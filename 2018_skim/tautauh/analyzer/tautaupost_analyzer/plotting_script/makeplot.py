...
#print("F_bkg in "+dirname[2]+"data_obs_"+histoname+"_fr")
F_bkg=OutFile.Get(dirname[2]+"data_obs_"+histoname+"_fr")
#print(F_bkg.Integral())
...
if(OutFile.Get(dirname[2]+"GluGluH_"+histoname+"_fr")):
  F_bkg.Add(GluGluH_bkg, -1)
if(OutFile.Get(dirname[2]+"ST_t_"+histoname+"_fr")):
  F_bkg.Add(ST_t_bkg, -1)
if(OutFile.Get(dirname[2]+"TT_"+histoname+"_fr")):
  F_bkg.Add(TT_bkg, -1)
...
uncF = F_bkg.Clone()
uncF.Reset("ICES")
xBins = F_bkg.GetNbinsX()
for ibin in range(1, xBins+1):
  content = F_bkg.GetBinContent(ibin)
  #print(ibin, content ) 
  if content >= 0 :
    uncF.SetBinContent(ibin, content)
    uncF.SetBinError(ibin, sqrt(content))
F_bkg = uncF

sampleList    = [Data_hist,   GluGluH_hist , ZLL_hist , ZTT_hist,  F_bkg ]
sampleListRef = ['Data_hist', 'GluGluH_hist', 'ZLL_hist', 'ZTT_hist', 'F_bkg']
...
if histoname=='cutflow_n' :
  with open('eventYield.csv', mode='a') as yield_file:
    yield_write = csv.writer(yield_file, delimiter=',', quotechar='"')
    yield_write.writerow(['From Cutflow bin 7' ])
    yield_write.writerow(['Data_hist', Data_hist.GetBinContent(7) ])
    yield_write.writerow(['total MC', ZTT_hist.GetBinContent(7) + F_bkg.GetBinContent(7) + TT_hist.GetBinContent(7) +GluGluH_hist.GetBinContent(7) ])
    yield_write.writerow(['ZTT',  ZTT_hist.GetBinContent(7)  ])
    yield_write.writerow(['ZLL',  ZLL_hist.GetBinContent(7)  ])
    yield_write.writerow(['Fake', F_bkg.GetBinContent(7) ])
    yield_write.writerow(['TT',   TT_hist.GetBinContent(7) ])
    yield_write.writerow(['ggh',  GluGluH_hist.GetBinContent(7) ])
    #yield_write.writerow(['VV' ,  VV_hist.GetBinContent(7) ])
if histoname=='tau1Charge_5' :
  with open('eventYield.csv', mode='a') as yield_file:
    yield_write = csv.writer(yield_file, delimiter=',', quotechar='"')
