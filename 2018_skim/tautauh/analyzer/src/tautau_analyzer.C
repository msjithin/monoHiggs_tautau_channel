...
void tautau_analyzer::Loop(Long64_t maxEvents, int reportEvery, string SampleName)
{
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
      ...
      if(metFilters==0 )
	{
			  ...
			  if (pass3rdLeptonVeto)
			    {
			      ...
			      if ( tau_Charge->at(Tau1Index)*tau_Charge->at(Tau2Index)<0  
				   &&  (if_DY_Genmatching(Tau1Index, Tau2Index)==2 || if_DY_Genmatching(Tau1Index, Tau2Index)==2)  )
				{
				  ...
				  if ( MatchTriggerFilter(Tau1Index, Tau2Index) )
				    {
				      ...
				      if(debug)cout<<"this worked Line"<<__LINE__<<", SR opp charge passed"<<endl;
      ...
      if(debug)cout<<"signal region -  isolated begin "<<__LINE__<<endl;       
		  ...
		  if( tauCand.size() >=2 && tau1>-1 && tau2>-1)
		    {
		      ...
		      // 	    cout<<tau_Pt->at(tauCand[i])<< "  "<<tau_byRawDeepTau2017v2p1VSjet->at(tauCand[i])<< "  "<< tau_Charge->at(tauCand[i])<<endl;
		      // 	  cout<<endl;
		      // 	}
			...
			if( eVetoZTTp001dxyz(Tau1Index, Tau2Index)
			    ...
			    //&& my_genmatching_l1 >=5 && my_genmatching_l2 >=5
			    && passDiElectronVeto(0)==true
			    ) Ztt_selector=true;
		      }
		      else
			Ztt_selector=true;
		      ...
		      if ( TriggerSelection(my_tau1P4, my_tau2P4) )
                        {
			  //if(Ztt_selector) 
			  if (pass3rdLeptonVeto && (if_DY_Genmatching(Tau1Index, Tau2Index)==1 || if_DY_Genmatching(Tau1Index, Tau2Index)==3) )
			    {
				...
				
				{
				  ...
				  if ( MatchTriggerFilter(Tau1Index, Tau2Index) )
					...
					applySf=  getScaleFactors( my_tau1P4.Pt(),
				      ...
				      //cout<<" sf : "<<applySf <<endl;
				      event_weight = event_weight * applySf;
				      // if(found_DYjet_sample && (my_tau1P4+my_tau2P4).M()<90 )
				      // 	{
				      // 	  event_weight = event_weight * dy_corr->Eval((my_tau1P4+my_tau2P4).M());
				      // 	}
				      ...
				      if( thirdLeptonVeto(Tau1Index, Tau2Index)  )
					{
					  ...
					  fillHist("4",  Tau1Index, Tau2Index, false, event_weight);
	      ...
	      if( tauCand.size()>0 )
		{
		  ...
		  if( tauCand.size()>=2 && leadingtau>-1 && subleadingtau>-1 ) 
		    { 
		      ...
		      // if(tauCand.size()>2){
		      // 	for(int i=0; i<tauCand.size(); i++){
		      // 	  cout<<"tau "<<tau_Pt->at(tauCand[i])<<" "
		      // 	      << tau_byRawDeepTau2017v2p1VSjet->at(tauCand[i])<<" "
		      // 	      <<tau_Charge->at(tauCand[i])<<" "
		      // 	      <<tau_byMediumDeepTau2017v2p1VSjet->at(tauCand[i])<<" "
		      // 	      <<tau_byVVVLooseDeepTau2017v2p1VSjet->at(tauCand[i])<<" "
		      // 	      <<endl;
		      // 	}
		      // 	cout<<" "<<endl;
		      // }
		      // // cout<<"comapare tau "<<tau_Pt->at(leadingtau)<<" "<<tau_byRawDeepTau2017v2p1VSjet->at(leadingtau)<<" "<<tau_Charge->at(leadingtau)<<endl;
		      // // cout<<"         tau "<<tau_Pt->at(subleadingtau)<<" "<<tau_byRawDeepTau2017v2p1VSjet->at(subleadingtau)<<" "<<tau_Charge->at(subleadingtau)<<endl;
  ...
  std::cout<<"ntt = "<<ntt<<endl;
  std::cout<<"nll = "<<nll<<endl;
	  ...
	  for(int i=0; i<tauIndices.size(); i++)
	    {
		  ...
		  && tau_Charge->at(tauIndices[ai])*tau_Charge->at(tauIndices[i])<0
		  )
		{
...
double tautau_analyzer::getScaleFactors(  double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  // cout<<" med vbf "<<tau1dm<<"  "<< tau1pt<<"  "<<TriggerSF_med_vbf.get_sf(tau1dm, tau1pt);
  // cout<<" med "<<tau1dm<<"  "<< tau1pt<<"  "<<TriggerSF_med.get_sf(tau1dm, tau1pt);
  // cout<<" vl vbf "<<tau1dm<<"  "<< tau1pt<<"  "<<TriggerSF_vl_vbf.get_sf(tau1dm, tau1pt);
  // cout<<" vl "<<tau1dm<<"  "<< tau1pt<<"  "<<TriggerSF_vl.get_sf(tau1dm, tau1pt);
  // cout<<"\n"<<endl;
  // if(isVBF()==true)
  //   cout<<"vbf category"<<endl;
  // if(isVBF()==true)
  //   {
  //     sf_tauTrg = TriggerSF_med_vbf.get_sf(tau1dm, tau1pt) * TriggerSF_med_vbf.get_sf(tau2dm, tau2pt);
  //     if(isFakebkg)
  // 	sf_tauTrg = TriggerSF_vvvl_vbf.get_sf(tau1dm, tau1pt) * TriggerSF_med_vbf.get_sf(tau2dm, tau2pt);
  //   }
  // else
  {
    sf_tauTrg = TriggerSF_med.get_sf(tau1dm, tau1pt) * TriggerSF_med.get_sf(tau2dm, tau2pt);
    if(isFakebkg)
      sf_tauTrg = TriggerSF_vvvl.get_sf(tau1dm, tau1pt) * TriggerSF_med.get_sf(tau2dm, tau2pt);
  }

  //cout <<TriggerSF_med.get_sf(tau1dm, tau1pt)<<" "<<TriggerSF_med.get_sf_eff(tau1dm, tau1pt)<<endl;
  // if(  my_genmatching_l1<2 || my_genmatching_l2<5 )
  // {  ///  muons to pass deep tau
  //   if(tau1dm==0 ){
  //     if(abs(tau1eta) < 0.4 ) sf_fakeMu=1.08;
  //     else if(abs(tau1eta) > 0.4 && abs(tau1eta) < 0.8 ) sf_fakeMu=0.78;
  //     else if(abs(tau1eta) > 0.8 && abs(tau1eta) < 1.2 ) sf_fakeMu=0.77;
  //     else if(abs(tau1eta) > 1.2 && abs(tau1eta) < 1.7 ) sf_fakeMu=0.75;
  ...
  //   }
  // }
  // if(  my_genmatching_l1<5 || my_genmatching_l2<5 )
  // {  ///  muons to pass deep tau
  //   if(tau1dm==0){
  //     if(abs(tau1eta) < 1.479 ) sf_fakeEle=1.09;
  //     else sf_fakeEle=0.80;
  //   }
  //   else  if(tau1dm==1){
  //     if(abs(tau1eta) < 1.479 ) sf_fakeEle=0.85;
  ...
  //rv_sf = rv_sf * sf_fakeMu * sf_fakeEle ;
  // if(isFakebkg)
  // //   sep = "\t\t";
  // if( (my_tau1P4+ my_tau2P4).M() < 100 )
  //   {
  //     printTabSeparated("rv_sf :",rv_sf, "isFake:", isFakebkg);
  //     printTabSeparated( "t1_trg_pog", t1_trg_pog ,t2_trg_pog );
  //     printTabSeparated( "sf_tauTrg : ",  TriggerSF_med.get_sf(tau1dm, tau1pt) , TriggerSF_med.get_sf(tau2dm, tau2pt));
  //     printTabSeparated( "tauid : ", t1_deeptauid, t2_deeptauid );
  //     printTabSeparated( "tauid : ", sf_tau1idSF_m , sf_tau2idSF_m );
  //     printTabSeparated( "sf_fakeEleMu", eleMuSF(my_genmatching_l1, tau1eta) , eleMuSF(my_genmatching_l2, tau2eta) , " genmatch", my_genmatching_l1, my_genmatching_l2);
...
int tautau_analyzer::if_DY_Genmatching(int tau1Index, int tau2Index){
  // if(!is_MC)
  //   return 1;
  if( found_DYjet_sample==false )
    return 1;
  else if(found_DYjet_sample==true){
    if(  my_genmatching_l1<5 && my_genmatching_l2<5 ) // dy -> ll genmatched
      return 2;
    if (  my_genmatching_l1>=5 && my_genmatching_l2>=5 ) // dy -> tautau genmatched
      return 3;
  }
  return 0;
...
bool tautau_analyzer::isVBF()
{
  ...
  if(jetCand.size()>=2)
    {
      if( abs(jetEta->at(jetCand[0]) - jetEta->at(jetCand[1])) > 2.5 )
	{
	  if( (my_tau1P4 + my_tau2P4 + my_metP4).Pt() > 100 )
	    return true;
	}
    }
  return false;
...
bool tautau_analyzer::isZeroJet()
{
  if(my_njets==0)
    return true;
