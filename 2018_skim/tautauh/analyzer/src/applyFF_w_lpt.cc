...
public:
  ...
  float get_ff(float pt, float mt, float mvis, float msv, float lpt, float met, int njets, int tau_dm , float frac_tt, float frac_qcd, float frac_w, TString shift){
    ...
    // cout<<"inputs : "<<pt<<" "<<lpt<<"  " << mvis<<"  "<<njets<<endl;
    // cout<<"rawff  "<<ff_qcd
    // 	<<"    "<< get_raw_FF(pt, ff_qcd_0jet) 
    // 	<<"    "<< get_raw_FF(pt, ff_qcd_1jet)
    // 	<<"    "<< get_raw_FF(pt, ff_qcd_2jet)<<endl;

    // cout<<"       "<<ff_qcd
    // 	<<"    "<< get_BinContent(ff_qcd_0jet_tt, pt) 
    // 	<<"    "<< get_BinContent(ff_qcd_1jet_tt, pt)
    // 	<<"    "<< get_BinContent(ff_qcd_2jet_tt, pt)
    // 	<<endl;
    // cout<<"\n"<<endl;
    ...
    //cout<<lpt <<"  "<< tau2Pt_corr->Eval(lpt)<<endl;
