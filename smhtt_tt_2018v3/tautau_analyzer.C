  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
    {
      ...
      //6: HLT_DoubleMedium ChargedIsoPFTau40_Trk1_TightID_eta2p1_Reg v --- only data run < 317509
      //7: HLT_DoubleTight ChargedIsoPFTau40_Trk1_eta2p1_Reg_v  --- only data run < 317509
      //5: HLT_DoubleTight ChargedIsoPF Tau35_Trk1_TightID_eta2p1_Reg_v --- only data run < 317509
      //17 :HLT_DoubleTightChargedIsoPFTauHPS40_Trk1_eta2p1_Reg_v --- MC and data run >= 317509
      ...
      if(metFilters==0 )
	{
	  ...
	  if( passTauTrigger )
		...
		{ 
		  ...
		  makeMyPlot("c", -1 ,-1,event_weight);
		  // if(tauCand.size()>=3)
		  //   {
		  //     for(int i=0; i<tauCand.size(); i++)
		  // 	{
		  // 	  cout<<tauCand[i]<<"  tau pt="<<tau_Pt->at(tauCand[i])<<"  "<<tau_byRawDeepTau2017v2p1VSjet->at(tauCand[i])<<"  "<<tau_Charge->at(tauCand[i])<<endl;
			  
		  // 	}
		  //   }
		  int tau1 = -1; int tau2 = -1;
		  sortTauIndices(tauCand,tau1, tau2 );
		  //tauCand = getTauCand(40.0,2.1);
		  if( tauCand.size()>=2  && tau1>-1 && tau2>-1 ) 
		    {
		      ...
		      // if(tauCand.size()>=3)
		      // 	{
		      // 	  cout<<"compare with tau1 "<<tau_Pt->at(tau1)<<"  "<<tau_byRawDeepTau2017v2p1VSjet->at(tau1)<<"  "<<tau_Charge->at(tau1)<<endl;
		      // 	  cout<<"             tau2 "<<tau_Pt->at(tau2)<<"  "<<tau_byRawDeepTau2017v2p1VSjet->at(tau2)<<"  "<<tau_Charge->at(tau2)<<endl;
		      // 	}
		      setMyEleTau(tau1, tau2); 
		      // from here we can use my_eleP4, my_tauP4, my_metP4, etc
		      makeMyPlot("d", Tau1Index,Tau2Index,event_weight);
		      
		      // printTabSeparated("index", Tau1Index,Tau2Index);
		      // printTabSeparated(  my_tau1P4.Pt(),  my_tau2P4.Pt() ) ; 
		      // printTabSeparated(  tau_byRawDeepTau2017v2p1VSjet->at(Tau1Index), tau_byRawDeepTau2017v2p1VSjet->at(Tau2Index) , "\n") ; 


		      //if(found_DYjet_sample){	
		      if( eVetoZTTp001dxyz(Tau1Index, Tau2Index)
			  && mVetoZTTp001dxyz(Tau1Index, Tau2Index)
			  && passDiElectronVeto()==true
			  ) Ztt_selector=true;
		      //}
		      //else
		      //Ztt_selector=true;
				  ...
				  if ( MatchTriggerFilter(Tau1Index, Tau2Index) )
				    {
				      ...
				      // 				   );
				      // cout<<"tau iso : "<<tau_byRawDeepTau2017v2p1VSjet->at(Tau1Index)<<" "<<tau_byRawDeepTau2017v2p1VSjet->at(Tau2Index)<<endl;
				      // cout<<"tau pt  : "<<my_tau1P4.Pt()<<" "<<my_tau2P4.Pt()<<" \n "<<endl;
				      
				      // if(debug)cout<<" sf : "<<getScaleFactors( EleIndex[0] , TauIndex[0] , false , is_MC , debug ) <<endl;
				      event_weight = event_weight * applySf;
				      //fillHist("3_dyll",  Tau1Index, Tau2Index, false, event_weight);
				      makeMyPlot("f", Tau1Index, Tau2Index,event_weight);
				      if( thirdLeptonVeto(Tau1Index, Tau2Index)  )
					{
					  nPassedThirdLepVeto+=event_weight;
					  //fillHist("4_dyll",  Tau1Index, Tau2Index, false, event_weight);
					  ...
					  //if( pass_bjet_veto )
					  {
						...
						plotFill("tauIso1_sr",  tau_byMediumDeepTau2017v2p1VSjet->at(Tau1Index) ,  4, 0, 2, event_weight);
						plotFill("tauIso2_sr",  tau_byMediumDeepTau2017v2p1VSjet->at(Tau2Index) ,  4, 0, 2, event_weight);  
						plotFill("tau1rawiso", tau_byRawDeepTau2017v2p1VSjet->at(Tau1Index), 20, 0, 1,  event_weight);
					      }
					  }
					}
				    }
      ...
      if(metFilters==0)
	{
	  ...
	  if( passTauTrigger  )
	      ...
	      if( tauCand.size()>0 )
		{
		  ...
		  makeMyPlot("fc", -1 ,-1,event_weight);
		  ...
		  if( tauCand.size()>=2 && leadingtau>-1 && subleadingtau>-1 ) 
		    { 
		      ...
		      // printTabSeparated(  my_tau1P4.Pt(),  my_tau2P4.Pt() ) ; 
		      // printTabSeparated(  tau_byRawDeepTau2017v2p1VSjet->at(Tau1Index), tau_byRawDeepTau2017v2p1VSjet->at(Tau2Index) ) ; 
		      // printTabSeparated(  tau_byMediumDeepTau2017v2p1VSjet->at(Tau1Index), tau_byMediumDeepTau2017v2p1VSjet->at(Tau2Index), "\n") ;
		      ...
		      makeMyPlot("fd", Tau1Index,Tau2Index,event_weight);
		      ...
		      if ( TriggerSelection(my_tau1P4, my_tau2P4) )
			{
			  if(Ztt_selector)
				...
				{				  
				  ...
				  makeMyPlot("fe", Tau1Index, Tau2Index,event_weight);
				  if ( MatchTriggerFilter(Tau1Index, Tau2Index) )
				    {
				      ...
				      event_weight = event_weight * applySf;
				      //fillHist("3_fr",  Tau1Index, Tau2Index, true, event_weight);
				      //event_weight = event_weight* getFR(TauIndex);
				      makeMyPlot("ff", Tau1Index, Tau2Index,event_weight);
				      //makeTestPlot("f_fr", 0,0,0,event_weight);
				      if( thirdLeptonVeto(Tau1Index, Tau2Index) )
					{
					  nPassedThirdLepVeto_fr+=event_weight;
					  //fillHist("4_fr",  Tau1Index, Tau2Index, true, event_weight);
					  makeMyPlot("fg", Tau1Index, Tau2Index,event_weight);
					  ...
					  //if( pass_bjet_veto )
					  {
					    ...
					    makeMyPlot("fh", Tau1Index, Tau2Index,event_weight);
					    double deltaR = my_tau1P4.DeltaR(my_tau2P4);
					    if(deltaR > 0.5 )
					      {
						nDeltaRPassed_fr+=event_weight;
						//makeTestPlot("i_fr", 0,0,0,event_weight);
						makeMyPlot("fi", Tau1Index, Tau2Index,event_weight);
						if(debug)cout<<"this worked Line 442"<<endl;
						//fillHist("5_fr", Tau1Index, Tau2Index, true, event_weight);
						plotFill("tauIso1_aisr",  tau_byMediumDeepTau2017v2p1VSjet->at(Tau1Index) ,  4, 0, 2, event_weight);
                                                plotFill("tauIso2_aisr",  tau_byMediumDeepTau2017v2p1VSjet->at(Tau2Index) ,  4, 0, 2, event_weight);
						plotFill("tau1rawiso", tau_byRawDeepTau2017v2p1VSjet->at(Tau1Index), 20, 0, 1,  event_weight);
					      }
					  }
					}
...
std::vector<int> tautau_analyzer::getTauCand_noID(double tauPtCut, double tauEtaCut){
  ...
  //   iso_map.push_back({ abs(tau_byRawDeepTau2017v2p1VSjet->at(tmpCand[i])) , tmpCand[i] });
  // }
...
bool tautau_analyzer::passDiElectronVeto()
{
  ...
  for(int iEle=0;iEle<nEle;iEle++)
    {
      ...
      bool electronId =false;
      if( eleIDbit->at(iEle)>>8&1==1) electronId =true; // cut based electron id veto == 3
      bool relative_iso = false;    
      float relEleIso = ( elePFChIso->at(iEle) + max( elePFNeuIso->at(iEle) + elePFPhoIso->at(iEle) - 0.5 *elePFPUIso->at(iEle) , 0.0 )) / (elePt->at(iEle));
      if( relEleIso < 0.3 ) relative_iso = true;
      if( kinematic && electronId && relative_iso ){
...
double tautau_analyzer::getScaleFactors(  double tau1pt, double tau2pt, double tau1eta, double tau2eta, int taudm, int tau1GenMatch, bool isFakebkg)
{
  ...
  // double sf_tauTrg_vvvloose=1.0;
  // {
  //   ///for tau 1
  //   if(tau1dm==0 )  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm0 , tau1PtCheck);
  //   if(tau1dm==1 )  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm1 , tau1PtCheck);
  //   if(tau1dm==10)  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm10, tau1PtCheck);
  //   if(tau1dm==11)  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm11, tau1PtCheck);
  //   ///for tau 2
  //   if(tau2dm==0 )  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm0 , tau2PtCheck);
  //   if(tau2dm==1 )  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm1 , tau2PtCheck);
  //   if(tau2dm==10)  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm10, tau2PtCheck);
  //   if(tau2dm==11)  sf_tauTrg_vvvloose= sf_tauTrg_vvvloose*get_BinContent(h_tauTrgSF_AI_dm1, tau2PtCheck);
  // }
  ...
  sf_tauTrg_vvvloose= tauTrgsf_vl->getSF(my_tau2P4.Pt(), tau2dm,0) * tauTrgsf_vl->getSF(my_tau1P4.Pt(), tau1dm,0);
  ...
  rv_sf = sf_tau1idSF_m * sf_tau2idSF_m * higgPt_weight * sf_tauTrg * sf_htt_tau1 * sf_htt_tau2 * zptmass_weight ;
  
  if(isFakebkg)
    rv_sf = rv_sf * sf_tau1idSF_vvvl * sf_tau2idSF_vvvl;
  // printTabSeparated("rv_sf :",rv_sf);
  // printTabSeparated("sf_htt_tau : ",sf_htt_tau1 , sf_htt_tau2 );
  // printTabSeparated("tauid : ", sf_tau1idSF_m, sf_tau2idSF_m );
  // printTabSeparated("tau trg : ", sf_tauTrg , sf_tau1Trg , sf_tau2Trg );
  // printTabSeparated("higgPt_weight ",higgPt_weight, zptmass_weight);
  // printTabSeparated("top_pt_weight ",top_pt_weight, "\n");
  if(rv_sf>0)
    return rv_sf;
  else
    return 1.0;
...
void tautau_analyzer::fillHist( string histNumber , int tau1Index, int tau2Index, bool isFakeBkg, float event_weight){
  ...
  plotFill("tau1rawiso_"+hNumber, tau_byRawDeepTau2017v2p1VSjet->at(tau1Index), 20, 0, 1,  event_weight);
  ...
  plotFill("tau2rawiso_"+hNumber, tau_byRawDeepTau2017v2p1VSjet->at(tau2Index), 20, 0, 1,  event_weight);
  ...
  plotFill("electronVeto_"+hNumber,  eVetoZTTp001dxyz(tau1Index, tau2Index) , 4, 0, 2,  event_weight);
  plotFill("muonVeto_"+hNumber,  mVetoZTTp001dxyz(tau1Index, tau2Index)     , 4, 0, 2,  event_weight);
  ...
  plotFill("nVtx_"+hNumber,  nVtx , 20, 0, 60,  event_weight);
...
void tautau_analyzer::applyTauESCorrections(TLorentzVector tauP4, int tauIndex, TLorentzVector& tauP4Corr)
{
  ...
  if(is_MC)
  {
    //cout<<"tau es corr "<< get_BinContent( h_tauesSF, tau_DecayMode->at(tauIndex) )<< "      tau dm ="<<tau_DecayMode->at(tauIndex) <<endl;
    if (myGenMaching(tauIndex)>=5) tauP4Corr=tauP4*get_BinContent( h_tauesSF, tau_DecayMode->at(tauIndex));
    ...
    if (  (myGenMaching(tauIndex)==1 || myGenMaching(tauIndex)==3) && tau_DecayMode->at(tauIndex)==0 ) tauP4Corr=tauP4*0.969;
    ...
    if (tau_DecayMode->at(tauIndex) == 0 ) tauP4Corr.SetPtEtaPhiM(tauP4Corr.Pt(), tauP4Corr.Eta(), tauP4Corr.Phi(), 0.1349766);
...
{
  return abs(tau_byRawDeepTau2017v2p1VSjet->at(a)) > abs(tau_byRawDeepTau2017v2p1VSjet->at(b));
}
void tautau_analyzer::makeMyPlot( string histNumber , int tau1Index, int tau2Index, float event_weight){
  ...
  plotFill("tau1Iso_"+hNumber, tau_byMediumDeepTau2017v2p1VSjet->at(tau1Index), 4, -1, 2,  event_weight);
  plotFill("tau1rawiso_"+hNumber, tau_byRawDeepTau2017v2p1VSjet->at(tau1Index), 20, 0, 1,  event_weight);
  plotFill("tau2Iso_"+hNumber, tau_byMediumDeepTau2017v2p1VSjet->at(tau2Index), 4, -2, 2,  event_weight);
  plotFill("tau2rawiso_"+hNumber, tau_byRawDeepTau2017v2p1VSjet->at(tau2Index), 20, 0, 1,  event_weight);
...
void tautau_analyzer::sortTauIndices(vector<int> tauIndices , int& tau1 , int& tau2 ){  
  std::vector<std::pair<float, int>> plus;
  std::vector<std::pair<float, int>> minus;
  ...
  for(int i=0; i<tauIndices.size(); i++)
    {
      if(tau_Charge->at(tauIndices[i]) > 0 )
	plus.push_back({ tau_byRawDeepTau2017v2p1VSjet->at(tauIndices[i]) , tauIndices[i] });
      if(tau_Charge->at(tauIndices[i]) < 0 )
	minus.push_back({ tau_byRawDeepTau2017v2p1VSjet->at(tauIndices[i]) , tauIndices[i] });
    }
  if(plus.size()>0)
  ...
  if(plus.size()>0 && minus.size()>0)
    {
      ...
      // 	  std::cout <<"Plus "<< plus[i].second << " " << plus[i].first << " " ;
      // }
      // if(minus.size()>1){
      // 	for(int i=0; i<minus.size(); i++)
      // 	  std::cout <<"Minus "<< minus[i].second << " " << minus[i].first << " " ;
      // }
      // cout<<endl;
      tau1 = plus[0].second ; tau2 = minus[0].second;
      return;
    }
  else
    { tau1=-1; tau2=-1; return; }
